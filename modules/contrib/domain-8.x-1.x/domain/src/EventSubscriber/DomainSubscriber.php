...
class DomainSubscriber implements EventSubscriberInterface {
  ...
  /**
  * This custom add on will set the sitename according to the active domain name
  **/

  public function setSiteName(){
    $domain_name = \Drupal::service('domain.negotiator')->getActiveDomain()->get('name');
    if($domain_name){
      $overwrite_sitename = \Drupal::configFactory()->getEditable('system.site')->set('name', $domain_name)->save();
    }
  }
  ...
  public static function getSubscribedEvents() {
    ...
    $events[KernelEvents::REQUEST][] = array('setSiteName');
    return $events;
  }
